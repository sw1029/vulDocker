...
Last validated against: `python -m pytest -q tests`, `python -m pytest -q tests/test_ops_ci_*.py`, targeted repeatability/support regressions, and representative direct/repeatability/support execution checks on 2026-03-21
...
- Docker-enabled fixture-backed positive LLM-shaped lane: `python tests/e2e/run_case.py --case tests/e2e/cases/trusted-dynamic-sqli --mode deterministic --no-snapshot --output-dir /tmp/vuld_trusted_dynamic`
  current truth는 `runnable but not promotable`다. representative positive pair support review에서도 `authority_ready_bundle_count=2`, `reviewable_bundle_count=0`로 남는다.
