...
def test_case_chain_main_supports_direct_profile(tmp_path: Path) -> None:
    ...
    _write_executable(
        ...
        f"""#!/usr/bin/env bash
set -euo pipefail
source {str(REPO_ROOT / "ops/ci/lib_case_chain_main.sh")!r}
fake_runner() {{
...
case_chain_run_main direct {str(REPO_ROOT / "ops/ci")!r} alpha-case
""",
    )
...
def test_case_chain_main_supports_repeatability_profile(tmp_path: Path) -> None:
    probe = tmp_path / "probe.sh"
    capture = tmp_path / "capture.json"
...
case_chain_run_main repeatability {str(REPO_ROOT / "ops/ci")!r} alpha-case
""",
    )
