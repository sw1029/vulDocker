...
def test_operator_run_direct_named_preset_exports_env_and_invokes_preset(tmp_path: Path) -> None:
...
""",
    ...
    completed = subprocess.run(
        [
            ...
            "-lc",
            (
                f"source {str(REPO_ROOT / 'ops/ci/lib_operator_direct_named_preset.sh')!r}\n"
                ...
                f"operator_run_direct_named_preset VULD_TEST {str(REPO_ROOT / 'tests/e2e/cases')!r} /tmp/default-out {str(named_helper)!r} {str(preset_helper)!r} {str(direct_helper)!r} '' '' TEST build_positive_pair_case_specs trusted-case open-redirect-case"
            ),
        ],
        cwd=REPO_ROOT,
        text=True,
...
def test_operator_run_direct_named_preset_rejects_missing_preset_helper(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [
            ...
            (
                f"source {str(REPO_ROOT / 'ops/ci/lib_operator_direct_named_preset.sh')!r}\n"
                f"operator_run_direct_named_preset VULD_TEST {str(REPO_ROOT / 'tests/e2e/cases')!r} /tmp/default-out {str(named_helper)!r} {str(missing)!r} {str(direct_helper)!r} '' '' TEST build_positive_pair_case_specs trusted-case open-redirect-case"
            ),
        ],
        cwd=REPO_ROOT,
        text=True,
