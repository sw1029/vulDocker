...
def test_positive_pair_promotion_check_supports_case_root_output_and_python_override(tmp_path: Path) -> None:
...
""",
    ...
    env = os.environ.copy()
    env["VULD_POSITIVE_PAIR_PYTHON_BIN"] = str(fake_python)
    env["VULD_POSITIVE_PAIR_CASES_ROOT"] = str(cases_root)
    env["VULD_POSITIVE_PAIR_OUTPUT_ROOT"] = str(output_root)

    completed = subprocess.run(
        ["bash", str(REPO_ROOT / "ops/ci/run_positive_pair_promotion_check.sh")],
        cwd=REPO_ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert "[PROMOTION] completed" in completed.stdout
    assert f"[PROMOTION] trusted_out={output_root / 'repeat_trusted_dynamic'}" in completed.stdout
    assert f"[PROMOTION] open_redirect_out={output_root / 'repeat_open_redirect_dynamic'}" in completed.stdout
    assert f"[PROMOTION] review_out={output_root / 'support_review_positive_pair.json'}" in completed.stdout
    ...
    assert (output_root / "support_review_positive_pair.json").exists()
    ...
    assert calls[2][-2:] == ["--output", str(output_root / "support_review_positive_pair.json")]


def test_positive_pair_promotion_check_supports_support_helper_override(tmp_path: Path) -> None:
...
""",
    ...
    env = os.environ.copy()
    env["VULD_POSITIVE_PAIR_SUPPORT_HELPER"] = str(fake_helper)
    env["VULD_POSITIVE_PAIR_PYTHON_BIN"] = "/tmp/fake_python"
    env["VULD_POSITIVE_PAIR_CASES_ROOT"] = "/tmp/fake_cases"
    env["VULD_POSITIVE_PAIR_OUTPUT_ROOT"] = "/tmp/fake_output"
    env["VULD_POSITIVE_PAIR_MODE"] = "diverse"
    env["VULD_POSITIVE_PAIR_NO_SNAPSHOT"] = "0"
    env["VULD_POSITIVE_PAIR_PERMISSION_ARTIFACT_NAME"] = "promotion_permission_marker.txt"
    env["VULD_POSITIVE_PAIR_PERMISSION_SUMMARY_NAME"] = "promotion_permission_summary.json"
    env["VULD_POSITIVE_PAIR_DOCKER_RETRY_COUNT"] = "4"
    env["VULD_POSITIVE_PAIR_DOCKER_RETRY_DELAY_SEC"] = "0"
    env["VULD_POSITIVE_PAIR_TRUSTED_CASE"] = "trusted-case"
    env["VULD_POSITIVE_PAIR_OPEN_REDIRECT_CASE"] = "open-redirect-case"
    ...
    assert payload["env"] == {
        ...
        "VULD_SUPPORT_WORKFLOW_NO_SNAPSHOT": "0",
        "VULD_SUPPORT_WORKFLOW_PERMISSION_ARTIFACT_NAME": "promotion_permission_marker.txt",
        "VULD_SUPPORT_WORKFLOW_PERMISSION_SUMMARY_NAME": "promotion_permission_summary.json",
        "VULD_SUPPORT_WORKFLOW_DOCKER_RETRY_COUNT": "4",
        "VULD_SUPPORT_WORKFLOW_DOCKER_RETRY_DELAY_SEC": "0",
        "VULD_SUPPORT_WORKFLOW_REVIEW_OUTPUT_NAME": "support_review_positive_pair.json",
    }


def test_positive_pair_promotion_check_supports_named_support_helper_override(tmp_path: Path) -> None:
    capture_path = tmp_path / "helper_capture.json"
...
payload = {{
    ...
    "env": {{
        ...
        "VULD_NAMED_SUPPORT_REVIEW_OUTPUT_NAME": os.environ.get("VULD_NAMED_SUPPORT_REVIEW_OUTPUT_NAME"),
    }},
}}
...
""",
    ...
    env = os.environ.copy()
    env["VULD_POSITIVE_PAIR_NAMED_SUPPORT_HELPER"] = str(fake_helper)
    env["VULD_POSITIVE_PAIR_SUPPORT_HELPER"] = "/tmp/support_helper"
    env["VULD_POSITIVE_PAIR_PYTHON_BIN"] = "/tmp/fake-python"
    env["VULD_POSITIVE_PAIR_CASES_ROOT"] = "/tmp/fake-cases"
    env["VULD_POSITIVE_PAIR_OUTPUT_ROOT"] = "/tmp/fake-output"
    env["VULD_POSITIVE_PAIR_MODE"] = "diverse"
    env["VULD_POSITIVE_PAIR_NO_SNAPSHOT"] = "0"
    env["VULD_POSITIVE_PAIR_PERMISSION_ARTIFACT_NAME"] = "promotion_permission_marker.txt"
    env["VULD_POSITIVE_PAIR_PERMISSION_SUMMARY_NAME"] = "promotion_permission_summary.json"
    ...
    assert payload["env"] == {
        ...
        "VULD_NAMED_SUPPORT_PERMISSION_ARTIFACT_NAME": "promotion_permission_marker.txt",
        "VULD_NAMED_SUPPORT_PERMISSION_SUMMARY_NAME": "promotion_permission_summary.json",
        "VULD_NAMED_SUPPORT_DOCKER_RETRY_COUNT": "4",
        "VULD_NAMED_SUPPORT_DOCKER_RETRY_DELAY_SEC": "0",
        "VULD_NAMED_SUPPORT_REVIEW_OUTPUT_NAME": "support_review_positive_pair.json",
    }


def test_positive_pair_promotion_check_supports_preset_helper_override(tmp_path: Path) -> None:
...
""",
    ...
    env = os.environ.copy()
    env["VULD_POSITIVE_PAIR_PRESET_HELPER"] = str(fake_helper)
    env["VULD_POSITIVE_PAIR_NAMED_SUPPORT_HELPER"] = "/tmp/named-support"
    env["VULD_POSITIVE_PAIR_TRUSTED_CASE"] = "trusted-case"
    env["VULD_POSITIVE_PAIR_OPEN_REDIRECT_CASE"] = "open-redirect-case"
    ...
    assert payload["env"] == {
        ...
        "VULD_NAMED_PRESET_LOG_PREFIX": "PROMOTION",
    }
