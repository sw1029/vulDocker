...
def test_case_chain_script_runner_supports_direct_wrapper(tmp_path: Path) -> None:
...
""",
    ...
    _write_executable(
        ...
        f"""#!/usr/bin/env bash
set -euo pipefail
source {str(REPO_ROOT / "ops/ci/lib_case_chain_script_runner.sh")!r}
...
""",
    ...
    env = os.environ.copy()
    env["VULD_TEST_PYTHON_BIN"] = str(fake_python)
    env["VULD_TEST_CASES_ROOT"] = str(cases_root)
    env["VULD_TEST_OUTPUT_ROOT"] = str(output_root)
...
def test_case_chain_script_runner_supports_repeatability_wrapper(tmp_path: Path) -> None:
    cases_root = tmp_path / "cases"
    _write_case(cases_root / "alpha-case", with_expectations=True)
    output_root = tmp_path / "outputs"
    capture_path = tmp_path / "python_calls.json"
...
""",
    ...
    env["VULD_TEST_CASES_ROOT"] = str(cases_root)
    env["VULD_TEST_OUTPUT_ROOT"] = str(output_root)
    env["VULD_TEST_RUN_DIRS_FILE"] = str(run_dirs_file)
