...
def test_case_chain_entrypoint_compat_supports_named_profile_dispatch(
    tmp_path: Path,
) -> None:
    ...
    _write_executable(
        ...
        f"""#!/usr/bin/env bash
set -euo pipefail
source {str(REPO_ROOT / "ops/ci/lib_case_chain_entrypoint_compat.sh")!r}
fake_runner() {{
...
case_chain_run_named_entrypoint direct {str(REPO_ROOT / "ops/ci")!r} alpha-case
""",
    )
...
def test_case_chain_entrypoint_compat_supports_repeatability_entrypoint(
    tmp_path: Path,
) -> None:
    probe = tmp_path / "probe.sh"
    capture = tmp_path / "capture.json"
...
def test_case_chain_entrypoint_compat_rejects_unknown_profile(tmp_path: Path) -> None:
    probe = tmp_path / "probe.sh"
...
set -euo pipefail
source {str(REPO_ROOT / "ops/ci/lib_case_chain_entrypoint_compat.sh")!r}
case_chain_run_named_entrypoint unknown {str(REPO_ROOT / "ops/ci")!r} alpha-case
""",
    )
